...
const completeTaskLink = document.querySelectorAll(".completeTaskLink");
...
const completeModal = document.querySelectorAll(".complete-modal");
...
const completeCloseBtn = document.querySelectorAll(".complete-no-button");
...
for (let i = 0; i < deleteTaskLink.length; i++) {
  ...
  completeTaskLink[i].addEventListener("click", () => {
    completeModal[i].style.display = "flex";
  });
}
...
for (let i = 0; i < deleteCloseBtn.length; i++) {
  ...
  completeCloseBtn[i].addEventListener("click", () => {
    completeModal[i].style.display = "none";
  });
}
...
for (let i = 0; i < deleteModal.length; i++) {
  window.addEventListener("click", (event) => {
    ...
    if (event.target == completeModal[i]) {
      completeModal[i].style.display = "none";
    }
  });
}
